...
#include <string.h>
#include <stdlib.h>
#define CALCULATE_RV 1

void TTVFast(double *params,double dt, double Time, double total,int n_planets, CalcTransit *transit, CalcRV *RV_struct,int n,int m, int input, char coords_fileName[100]);
...
int main(int argc, char **argv)
{
  ...
  fscanf(setup_file, "%s", coords_fileName);
  fclose(setup_file);
  ...
  TTVFast(p,dt,Time,total,nplanets,model,RV_model,RV_count,n_events,input_flag,coords_fileName);
