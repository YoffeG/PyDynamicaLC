...
status_t TTVFast_main(double *params,double dt, double Time, double total,int n_plan,CalcTransit *transit,CalcRV *RV_struct, int nRV, int n_events, int input_flag, char coords_fileName[100]);
...
void TTVFast(double *params,double dt, double Time, double total,int n_plan,CalcTransit *transit,CalcRV *RV_struct, int nRV, int n_events, int input_flag, char coords_fileName[100]) {
    status_t status = TTVFast_main(params, dt, Time, total, n_plan, transit, RV_struct, nRV, n_events, input_flag, coords_fileName);
    switch (status) {
        case STATUS_OK:
            break;
        ...
        for(i = 0;i<n_planets;i++){ /* for each planet */
            ...
            if(prev_dot[tplanet]<0 && curr_dot[tplanet]>0 && curr_z[tplanet]>0){
                ...
                if(dot1 < 0 && dot2 >0){
                    ...
                    Coords_output = fopen(coords_fileName, "a");
//                    Coords_output = fopen(snprintf(koi_str, 4, "%d", koi), "a");
                    fprintf(Coords_output, "%.15lf %.15lf %.15lf %.15lf %.15lf %.15lf %.15lf %d \n", x1, y1, z1, xd1, yd1, zd1, Time, i);
                    fclose(Coords_output);

//                    char koi_str[3];
//
//                    Coords_output = fopen(snprintf(koi_str, 3, "%d", koi), "a");
//                    fprintf(Coords_output, "%.15lf %.15lf %.15lf %.15lf %.15lf %.15lf %.15lf %d \n", x1, y1, z1, xd1, yd1, zd1, Time, i);
//                    fclose(Coords_output);
//                    //                    printf("%f \n", rsky);
